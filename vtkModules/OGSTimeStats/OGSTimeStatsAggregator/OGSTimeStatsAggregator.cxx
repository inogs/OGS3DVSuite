/*=========================================================================
  ...
  Module:    OGSTimeStatsAggregator.cxx
...
#include "OGSTimeStatsAggregator.h"
...
vtkStandardNewMacro(OGSTimeStatsAggregator);

#ifdef PARAVIEW_USE_MPI
#include "vtkMultiProcessController.h"
vtkCxxSetObjectMacro(OGSTimeStatsAggregator, Controller,
...
//----------------------------------------------------------------------------
OGSTimeStatsAggregator::OGSTimeStatsAggregator() {
  this->sum_weights = 0.;
...
//----------------------------------------------------------------------------
OGSTimeStatsAggregator::~OGSTimeStatsAggregator() {
  OGS::Time::deallocList(this->ReqList);
...
//----------------------------------------------------------------------------
int OGSTimeStatsAggregator::RequestInformation(
...
// Handle the parallel controller
  ...
  // this->instants.clear(); this->weights.clear(); // make sure vectors are
  // empty 		if
  //(TL.select(this->ReqList[ii],this->instants,this->weights) != TIME_ERR)
  // for (int jj=0; jj<this->instants.size();++jj)
  // printf("ind: %d, weight: %f, %s\n",this->instants[jj],this->weights[jj],
  //				this->TL[instants[jj]].as_string("%Y-%m-%d
  //%H:%M:%S").c_str()); 			else
  // printf("Error!\n");
  //	}
  ...
  timeSteps = new double[this->ReqList.size()];
  for (int ii = 0; ii < this->ReqList.size(); ii++) {
...
//------------------------------------------------------------------------------
int OGSTimeStatsAggregator::RequestUpdateExtent(
...
//----------------------------------------------------------------------------
int OGSTimeStatsAggregator::RequestData(vtkInformation *request,
  ...
  if (this->CurrentTimeIndex == 0) {
    ...
    if (this->procId == 0) {
      ...
      int ii_tstep = std::distance(
          timeSteps, std::find(timeSteps, timeSteps + this->ReqList.size(),
                               requestedTimeValue));
      ...
      if (this->TL.select(this->ReqList[ii_tstep], this->instants,
                          this->weights) == TIME_ERR) {
        vtkErrorMacro(
...
//----------------------------------------------------------------------------
int OGSTimeStatsAggregator::PipelineIterationAlgorithm(vtkInformation *request,
                                                       vtkDataSet *inData,
                                                       vtkDataSet *outData) {
// Stop all threads except from the master to execute
  ...
  if (this->CurrentTimeIndex == 0) {
    ...
    if (vtkmetadata == NULL) {
      ...
      // Set the datevec
      std::string aux_str =
          std::to_string(this->ReqList.size()) + std::string(";");
      for (int it = 0; it < this->ReqList.size(); it++) {
        aux_str += this->ReqList[it]->interval().get_start_time().as_string(
                       "%Y-%m-%d %H:%M:%S") +
                   std::string(";");
      }
...
//----------------------------------------------------------------------------
int OGSTimeStatsAggregator::FileIterationAlgorithm(vtkInformation *request,
                                                   vtkDataSet *inData,
                                                   vtkDataSet *outData) {
  // Check inputs
