/*=========================================================================
  ...
  Module:    OGSSpatialStatsFromFile.cxx
...
#include "OGSSpatialStatsFromFile.h"
...
vtkCxxSetObjectMacro(OGSSpatialStatsFromFile, Controller,
                     vtkMultiProcessController);
#endif

vtkStandardNewMacro(OGSSpatialStatsFromFile);
...
#define STTIND(bId, cId, kk, sId, ns, nz, nc) \
  ((ns) * (nz) * (nc) * (bId) + (ns) * (nz) * (cId) + (ns) * (kk) + (sId))

//----------------------------------------------------------------------------
OGSSpatialStatsFromFile::OGSSpatialStatsFromFile() {
...
//----------------------------------------------------------------------------
int OGSSpatialStatsFromFile::RequestData(vtkInformation *vtkNotUsed(request),
...
// Stop all threads except from the master to execute
  ...
  std::string filename = std::string(this->FolderName) + std::string("/ave.") +
  ...
  // Basins mask
  vtkMask =
      VTKMASK::SafeDownCast(input->GetCellData()->GetArray(this->bmask_field));
  output->GetCellData()->AddArray(vtkMask);
  bmask = VTK::createFieldfromVTK<VTKMASK, FLDMASK>(vtkMask);
  // Coast mask
  vtkMask =
      VTKMASK::SafeDownCast(input->GetCellData()->GetArray(this->cmask_field));
  output->GetCellData()->AddArray(vtkMask);
  cmask = VTK::createFieldfromVTK<VTKMASK, FLDMASK>(vtkMask);

  // Copy e1, e2 and e3
  VTKARRAY *vtkArray;
  vtkArray = VTKARRAY::SafeDownCast(input->GetCellData()->GetArray("e1"));
  output->GetCellData()->AddArray(vtkArray);
  vtkArray = VTKARRAY::SafeDownCast(input->GetCellData()->GetArray("e2"));
  output->GetCellData()->AddArray(vtkArray);
  vtkArray = VTKARRAY::SafeDownCast(input->GetCellData()->GetArray("e3"));
  output->GetCellData()->AddArray(vtkArray);
  ...
  for (int varId = 0; varId < narrays; varId++) {
    ...
    OGS::field::Field<FLDARRAY> statProfile(
        nbasins * ncoasts * (nz - 1) * nstat, 1);

    if (OGS::NetCDF::readNetCDF(filename.c_str(), arrName.c_str(),
                                statProfile) != NETCDF_OK) {
    ...
    for (int statId = 0; statId < nstat; ++statId) {
      ...
      OGS::field::Field<FLDARRAY> statArray(array.get_n(), array.get_m());
...
#pragma omp parallel for collapse(3)
      for (int kk = 0; kk < nz - 1; ++kk) {
        for (int jj = 0; jj < ny - 1; ++jj) {
          for (int ii = 0; ii < nx - 1; ++ii) {
            ...
            // In which basin are we? (we need to loop the basins and find which
            // is true)
            int bId = 0;
            bool isbasin = false;
            ...
            statArray[pos][0] =
                (isbasin) ? statProfile[STTIND(bId, cId, kk, statId, nstat,
                                               nz - 1, ncoasts)][0]
                          : 0.;
          }
        }
      ...
      vtkstatArray = VTK::createVTKfromField<VTKARRAY, FLDARRAY>(
          statName.c_str(), statArray);
...
//----------------------------------------------------------------------------
void OGSSpatialStatsFromFile::DisableAllStatArrays() {
  this->StatDataArraySelection->DisableAllArrays();
}

void OGSSpatialStatsFromFile::EnableAllStatArrays() {
  this->StatDataArraySelection->EnableAllArrays();
}

int OGSSpatialStatsFromFile::GetNumberOfStatArrays() {
  return this->StatDataArraySelection->GetNumberOfArrays();
}

const char *OGSSpatialStatsFromFile::GetStatArrayName(int index) {
...
int OGSSpatialStatsFromFile::GetStatArrayIndex(const char *name) {
  return this->StatDataArraySelection->GetArrayIndex(name);
}

int OGSSpatialStatsFromFile::GetStatArrayStatus(const char *name) {
  return this->StatDataArraySelection->ArrayIsEnabled(name);
}

void OGSSpatialStatsFromFile::SetStatArrayStatus(const char *name, int status) {
  if (status)
