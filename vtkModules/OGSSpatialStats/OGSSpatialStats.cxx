...
vtkCxxSetObjectMacro(OGSSpatialStats, Controller, vtkMultiProcessController);
#endif

vtkStandardNewMacro(OGSSpatialStats);
...
//----------------------------------------------------------------------------
OGSSpatialStats::OGSSpatialStats() {
...
//----------------------------------------------------------------------------
int OGSSpatialStats::RequestInformation(
...
//----------------------------------------------------------------------------
int OGSSpatialStats::RequestData(vtkInformation *vtkNotUsed(request),
...
// Stop all threads except from the master to execute
  ...
  if (this->changing_mesh || this->xyz.isempty() || this->isReqInfo) {
    ...
    double dfact =
        (vtkmetadata != NULL) ? std::stod(vtkmetadata->GetValue(2)) : 1000.;
    ...
    this->xyz = (this->iscelld) ? getVTKCellCenters(input, dfact)
                                : getVTKCellPoints(input, dfact);

    // If the user has not inputed any depth level, make sure to clear the
    // vector
  ...
  vtke1 = (this->iscelld)
              ? VTKARRAY::SafeDownCast(input->GetCellData()->GetArray("e1"))
              : VTKARRAY::SafeDownCast(input->GetPointData()->GetArray("e1"));
  vtke2 = (this->iscelld)
              ? VTKARRAY::SafeDownCast(input->GetCellData()->GetArray("e2"))
              : VTKARRAY::SafeDownCast(input->GetPointData()->GetArray("e2"));
  vtke3 = (this->iscelld)
              ? VTKARRAY::SafeDownCast(input->GetCellData()->GetArray("e3"))
  ...
  OGS::field::Field<FLDARRAY> e1, e2, e3;
  ...
  // vtkcId2zId =
  // VTK::createVTKfromField<vtkIntArray,int>("cId2zId",this->cId2zId);
  ...
  for (int varId = 0; varId < narrays; ++varId) {
    ...
    for (int statId = 0; statId < nstat; ++statId) {
      ...
      if (!this->GetStatArrayStatus(statName.c_str())) continue;
      // Create empty field array
      OGS::field::Field<FLDARRAY> statArray(array.get_n(), array.get_m(), 0.);
      // Pair and insert to map
      mStatArray.insert(std::make_pair(statName, statArray));
    }

    // Also, as preprocess, create some maps to refer the data on the layers
...
#pragma omp parallel
    {
      for (int ii = OMP_THREAD_NUM; ii < array.get_n(); ii += OMP_NUM_THREADS) {
        // Set maps
        double w = (this->useVolume) ? e1[ii][0] * e2[ii][0] * e3[ii][0]
                                     : e1[ii][0] * e2[ii][0];
...
#pragma omp barrier
      ...
      for (int zId = OMP_THREAD_NUM; zId < this->zcoords.size();
           zId += OMP_NUM_THREADS) {
        std::vector<std::pair<FLDARRAY, int>> sortedValues(
            mValuesPerLayer[zId].size());
        ...
        // West, D. H. D. "Updating mean and variance estimates: An improved
        // method." Communications of the ACM 22.9 (1979): 532-535.
        double sum_weight = 0., meanval = 0., meanval_old = 0., stdval = 0.,
               maxval = -1.e20, minval = 1.e20;
        ...
        // Just execute this part of the code if the user has requested to
        // compute the percentiles. Otherwise, save a couple of for loops.
        double perc[] = {.05, .25, .50, .75, .95};
        double percval[] = {0., 0., 0., 0., 0.};

        if (this->GetStatArrayStatus("p05") ||
            this->GetStatArrayStatus("p50") ||
            this->GetStatArrayStatus("p75") ||
            this->GetStatArrayStatus("p95")) {
          // Order the values
          std::sort(sortedValues.begin(), sortedValues.end());

          // Second iteration on the layer, this time compute the percentile
          // weight
          double pweight = 0.;
          ...
          for (int ii = 0; ii < mValuesPerLayer[zId].size(); ++ii) {
            ...
            double aux = (pweight - .5 * mWeightPerLayer[zId][ind]) /
                         sum_weight;  // Reused variable
            percw[ii] = aux;
          }
          ...
          for (int pp = 0; pp < 5; pp++) {
            ...
            std::vector<FLDARRAY>::iterator lbound =
                std::lower_bound(percw.begin(), percw.end(), perc[pp]);
            ...
            if (s == 0) {
              percval[pp] = sortedValues[s].first;
              continue;
            }  // == sd[0]
            if (s == mValuesPerLayer[zId].size() - 1) {
              percval[pp] = sortedValues[s].first;
              continue;
            }  // == sd[n-1]
            ...
            percval[pp] =
                f1 * sortedValues[s - 1].first + f2 * sortedValues[s].first;
          }
        }
        ...
        for (int ii = 0; ii < mMeshPerLayer[zId].size(); ++ii) {
          std::map<std::string, OGS::field::Field<FLDARRAY>>::iterator iter;
          ...
          for (iter = mStatArray.begin(); iter != mStatArray.end(); ++iter) {
            ...
            switch (this->GetStatArrayIndex(iter->first.c_str())) {
              case 0:  // Mean
                iter->second[ind][0] = meanval;
                break;
              case 1:  // Std dev
                iter->second[ind][0] = stdval;
                break;
              case 2:  // Min
                iter->second[ind][0] = minval;
                break;
              case 3:  // p05
                iter->second[ind][0] = percval[0];
                break;
                ;
              case 4:  // p25
                iter->second[ind][0] = percval[1];
                break;
              case 5:  // p50
                iter->second[ind][0] = percval[2];
                break;
              case 6:  // p75
                iter->second[ind][0] = percval[3];
                break;
              case 7:  // p95
                iter->second[ind][0] = percval[4];
                break;
              case 8:  // Max
                iter->second[ind][0] = maxval;
                break;
            }
          }
        }
...
//----------------------------------------------------------------------------
void OGSSpatialStats::SetNumberOfDepthLevels(int n) {
...
void OGSSpatialStats::SetDepthLevels(int i, double value) {
  // We basically use this function to set zcoords
  this->zcoords.push_back(-value);
}

//----------------------------------------------------------------------------
void OGSSpatialStats::DisableAllStatArrays() {
  this->StatDataArraySelection->DisableAllArrays();
}

void OGSSpatialStats::EnableAllStatArrays() {
  this->StatDataArraySelection->EnableAllArrays();
}

int OGSSpatialStats::GetNumberOfStatArrays() {
  return this->StatDataArraySelection->GetNumberOfArrays();
}

const char *OGSSpatialStats::GetStatArrayName(int index) {
...
int OGSSpatialStats::GetStatArrayIndex(const char *name) {
  return this->StatDataArraySelection->GetArrayIndex(name);
}

int OGSSpatialStats::GetStatArrayStatus(const char *name) {
  return this->StatDataArraySelection->ArrayIsEnabled(name);
}

void OGSSpatialStats::SetStatArrayStatus(const char *name, int status) {
  if (status)
