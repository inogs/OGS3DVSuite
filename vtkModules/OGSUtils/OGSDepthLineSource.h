...
#ifndef OGSDepthLineSource_h
#define OGSDepthLineSource_h
...
class OGSDepthLineSource : public vtkPolyDataAlgorithm {
 public:
  static OGSDepthLineSource *New();
  vtkTypeMacro(OGSDepthLineSource, vtkPolyDataAlgorithm);
 ...
 protected:
  OGSDepthLineSource(int res = 1);
  ~OGSDepthLineSource() override;
  ...
  // NB: The Point1/Point2 definition of a single line segment is used by
  // default
  vtkPoints *Points;

 private:
  OGSDepthLineSource(const OGSDepthLineSource &) = delete;
  void operator=(const OGSDepthLineSource &) = delete;
};
