/*
        ...
        Usage:
                ...
                > A == B is testing size(A) == size(B), contents may be
   different!! > A != B is testing size(A) != size(B), contents may be
   different!!

    Functions:
        > size(i) or size(i,j) defines the size of the matrix
        > ij(i,j) or ij(i,j,val) lets grab a value using i,j indexes (from 1 to
   end) and also set a new value (val) at the i,j position > print(syle) prints
   the matrix in a nice printf style
...
template <class T>
class matrixMN {
  friend matrixMN<T> operator+(T val, const matrixMN<T> &mn) {
    matrixMN<T> out(mn.m, mn.n);
    for (int i = 0; i < mn.sz; ++i) {
      out.mat[i] = val + mn.mat[i];
    }
    return out;
  }
  friend matrixMN<T> operator-(T val, const matrixMN<T> &mn) {
 ...
 public:
  ...
  inline matrixMN(const int i) {
    size(i);
    fill((T)(0.));
  }
  inline matrixMN(const int i, const T val) {
    size(i);
    fill(val);
  }
  inline matrixMN(const int i, const T val[]) {
  ...
  inline matrixMN(const int i, const int j) {
    size(i, j);
    fill((T)(0.));
  }
  inline matrixMN(const int i, const int j, const T val) {
    size(i, j);
    fill(val);
  }
  inline matrixMN(const int i, const int j, const T val[]) {
  ...
  inline matrixMN(const matrixMN<T> &mn) {
    size(mn.m, mn.n);
    fill(mn.mat);
  }
  ...
  inline void size(const int i, const int j) {
    m = i;
    n = j;
    sz = i * j;
    bsz = -1.;
  ...
  inline void fill(const T val) {
    if (alloc) std::fill(mat, mat + sz, val);
  }
  inline void fill(const T val[]) {
    if (alloc) std::memcpy(mat, val, sz * sizeof(T));
  }
  inline void clear() {
    n = 0;
    m = 0;
  ...
  inline void ij(const int i, const int j, const T val) {
    mat[n * i + j] = val;
  }
  ...
  template <class U>
  inline matrixMN<U> convert() {
    matrixMN<U> A(n, m);
    std::copy(mat, mat + sz, A.data());
    return A;
  }
  ...
  inline matrixMN<double> schur(char sort, LAPACK_D_SELECT2 select,
                                matrixMN<double> &Z, double wr[], double wi[]);
  inline matrixMN<float> schur(char sort, LAPACK_D_SELECT2 select,
                               matrixMN<float> &Z, float wr[], float wi[]);

  // Operators
  inline matrixMN<T> &operator=(const matrixMN<T> &mn) {
    clear();
    size(mn.m, mn.n);
    fill(mn.mat);
    return (*this);
  }  // Equality of matrix sizes
  inline matrixMN<T> operator+(const T val) const {
    matrixMN<T> out(m, n);
    for (int i = 0; i < sz; ++i) {
      out.mat[i] = mat[i] + val;
    }
    return out;
  }  // Sum matrix and number at all points
  inline matrixMN<T> operator-(const T val) const {
    matrixMN<T> out(m, n);
    for (int i = 0; i < sz; ++i) {
      out.mat[i] = mat[i] - val;
    ...
    return out;
  }  // Substract matrix and number at all points
  inline matrixMN<T> operator*(const T val) const {
    matrixMN<T> out(m, n);
    for (int i = 0; i < sz; ++i) {
      out.mat[i] = mat[i] * val;
    ...
    return out;
  }  // Product matrix and number at all points
  inline matrixMN<T> operator/(const T val) const {
    matrixMN<T> out(m, n);
    for (int i = 0; i < sz; ++i) {
      out.mat[i] = mat[i] / val;
    ...
    return out;
  }  // Division matrix and number at all points
  inline matrixMN<T> operator+(const matrixMN<T> &mn) {
    matrixMN<T> out(m, n);
    if ((*this) == mn) {
      for (int i = 0; i < sz; ++i) {
        out.mat[i] = mat[i] + mn.mat[i];
      }
    }
    return out;
  }  // Sum of two matrices
  ...
  inline void operator+=(const matrixMN<T> &mn) {
    if ((*this) == mn) {
      for (int i = 0; i < sz; ++i) {
        mat[i] += mn.mat[i];
      }
  ...
  inline bool operator==(const matrixMN<T> &mn) const {
    return ((m == mn.m && n == mn.n) ? true : false);
  }  // This is used to test equality of sizes
...
inline matrixMN<T> matrixMN<T>::operator^(const matrixMN<T> &mn) {
...
#ifdef MKL
  cblas_dgemm(CblasRowMajor, CblasNoTrans, CblasNoTrans, this->m, mn.n, mn.m,
              1 this->mat, this->n, mn.mat, 1, out.mat, out.n);
#else
  if (this->m == 2 && this->n == 2) {
    ...
    out.ij(0, 0,
           this->ij(0, 0) * mn.ij(0, 0) + this->ij(0, 1) * mn.ij(1, 0) +
               this->ij(0, 2) * mn.ij(2, 0));
    out.ij(0, 1,
           this->ij(0, 0) * mn.ij(0, 1) + this->ij(0, 1) * mn.ij(1, 1) +
               this->ij(0, 2) * mn.ij(2, 1));
    out.ij(0, 2,
           this->ij(0, 0) * mn.ij(0, 2) + this->ij(0, 1) * mn.ij(1, 2) +
               this->ij(0, 2) * mn.ij(2, 2));
    out.ij(1, 0,
           this->ij(1, 0) * mn.ij(0, 0) + this->ij(1, 1) * mn.ij(1, 0) +
               this->ij(1, 2) * mn.ij(2, 0));
    out.ij(1, 1,
           this->ij(1, 0) * mn.ij(0, 1) + this->ij(1, 1) * mn.ij(1, 1) +
               this->ij(1, 2) * mn.ij(2, 1));
    out.ij(1, 2,
           this->ij(1, 0) * mn.ij(0, 2) + this->ij(1, 1) * mn.ij(1, 2) +
               this->ij(1, 2) * mn.ij(2, 2));
    out.ij(2, 0,
           this->ij(2, 0) * mn.ij(0, 0) + this->ij(2, 1) * mn.ij(1, 0) +
               this->ij(2, 2) * mn.ij(2, 0));
    out.ij(2, 1,
           this->ij(2, 0) * mn.ij(0, 1) + this->ij(2, 1) * mn.ij(1, 1) +
               this->ij(2, 2) * mn.ij(2, 1));
    out.ij(2, 2,
           this->ij(2, 0) * mn.ij(0, 2) + this->ij(2, 1) * mn.ij(1, 2) +
               this->ij(2, 2) * mn.ij(2, 2));
  } else {
...
inline void matrixMN<T>::print(const char *style) {
  if (alloc) {
    for (int i = 0; i < this->m; ++i) {
      std::printf("|");
      for (int j = 0; j < this->n; ++j) std::printf(style, this->ij(i, j));
      std::printf("|\n");
    }
  } else {
...
inline T matrixMN<T>::norm2() {  // Frobenius norm of a matrix squared.
  T norm2 = 0.;
  for (int i = 0; i < this->m; ++i) {
    for (int j = 0; j < this->n; ++j) norm2 += this->ij(i, j) * this->ij(i, j);
  }
  return norm2;
}
template <class T>
...
inline matrixMN<T> matrixMN<T>::t() {
  return (this->m == this->n && this->bsz > 0 && this->m > BLK_LIM)
             ? this->t_f()
             : this->t_n();
}
template <class T>
inline T *matrixMN<T>::diag() {
  ...
  if (this->m == this->n) {
    diag = new T[this->n];
    for (int i = 0; i < this->m; ++i) diag[i] = this->ij(i, i);
  }
  return diag;
}
template <class T>
inline T matrixMN<T>::tra() {
  T tra = 0.;
  if (this->m == this->n) {
    for (int i = 0; i < this->m; ++i) tra += this->ij(i, i);
  }
  return tra;
}
template <class T>
inline void matrixMN<T>::iden() {
  this->fill(0.);
  if (this->m == this->n) {
    for (int i = 0; i < this->m; ++i) this->ij(i, i, 1.);
  }
}
...
template <>
inline matrixMN<double> matrixMN<double>::eigen(double wr[], double wi[],
                                                matrixMN<double> &V) {
...
#ifdef USE_LAPACK
  LAPACKE_dgeev(LAPACK_MAJOR, 'N', 'V', A.get_n(), A.data(), A.get_m(), &wr[0],
                &wi[0], dummy, this->n, V.data(), this->n);
#endif
  // Deallocate memory
  delete[] dummy;
...
template <>
inline matrixMN<float> matrixMN<float>::eigen(float wr[], float wi[],
                                              matrixMN<float> &V) {
  if (this->n != this->m) return (*this);  // The matrix is not n-by-n
  // Eigen algorithm overwrites the input matrix, we will copy and return it
  // as the eigenvectors
...
// Call lapacke routine
#ifdef USE_LAPACK
  LAPACKE_sgeev(LAPACK_MAJOR, 'N', 'V', A.get_n(), A.data(), A.get_m(), &wr[0],
                &wi[0], dummy, this->n, V.data(), this->n);
#endif
  // Deallocate memory
  delete[] dummy;
...
inline matrixMN<T> matrixMN<T>::schur(char sort, LAPACK_D_SELECT2 select) {
  /*
          The routine computes for an n-by-n real/complex nonsymmetric matrix A,
     the eigenvalues, the real Schur form T, and, optionally, the matrix of
     Schur vectors Z. This gives the Schur factorization A = Z*T*ZH.

          Optionally, it also orders the eigenvalues on the diagonal of the
     real-Schur/Schur form so that selected eigenvalues are at the top left. The
     leading columns of Z then form an orthonormal basis for the invariant
...
template <>
inline matrixMN<double> matrixMN<double>::schur(char sort,
  ...
  /*
     ...
     Schur vectors Z. This gives the Schur factorization A = Z*T*ZH.
          ...
          Optionally, it also orders the eigenvalues on the diagonal of the
     real-Schur/Schur form so that selected eigenvalintues are at the top left.
...
// Call lapacke routine
#ifdef USE_LAPACK
  LAPACKE_dgees(LAPACK_MAJOR, 'V', sort, select, A.get_n(), A.data(), A.get_n(),
                &sdim, &wr[0], &wi[0], Z.data(), A.get_n());
#endif
...
template <>
inline matrixMN<float> matrixMN<float>::schur(char sort,
                                              LAPACK_D_SELECT2 select,
                                              matrixMN<float> &Z, float wr[],
                                              float wi[]) {
  /*
          The routine computes for an n-by-n real/complex nonsymmetric matrix A,
     the eigenvalues, the real Schur form T, and, optionally, the matrix of
  ...
  */
  ...
  LAPACKE_dgees(LAPACK_MAJOR, 'V', sort, select, A_aux.get_n(), A_aux.data(),
                A_aux.get_n(), &sdim, &wr_aux[0], &wi_aux[0], Z_aux.data(),
                A_aux.get_n());
  // Copy outputs
  std::copy(wr_aux, wr_aux + this->n, wr);
  std::copy(wi_aux, wi_aux + this->n, wi);
  A = A_aux.convert<float>();
  Z = Z_aux.convert<float>();
  delete[] wr_aux;
  delete[] wi_aux;
#endif
  // Return A as the real Schur form
