...
OGS::V3::V3v getVTKCellCenters(vtkDataSet *mesh, double fact) {
  // Define output array
  OGS::V3::V3v xyz(mesh->GetNumberOfCells());
...
#pragma omp parallel shared(mesh, xyz) firstprivate(fact)
  {
    ...
    for (unsigned int ii = OMP_THREAD_NUM; ii < xyz.len();
      ...
      for (int pId = 0; pId < ncellp; ++pId) {
        // Obtain point from VTK structure
        double pnt[3];
        cell->GetPoints()->GetPoint(pId, pnt);
        // Set the V3 structure
