  ...
  if (uniquevals.empty()) {
    ...
    for (itId = cId2zId.begin(), itxyz = xyz.begin(); itxyz != xyz.end();
      ...
      for (double zunique : uniquevals) {  // Range based foor loop here
        ...
        if (std::fabs(zunique - itxyz[2]) < epsi) {
          isunique = false;
          break;
        }
      }
      ...
      itId[0] = -1;
      for (unsigned int ii = 0; ii < uniquevals.size(); ii++)
        if (std::fabs(uniquevals[ii] - itxyz[2]) < epsi) itId[0] = (int)ii;
    }
  } else {
    ...
    for (itId = cId2zId.begin(), itxyz = xyz.begin(); itxyz != xyz.end();
      ...
      itId[0] = -1;
      for (unsigned int ii = 0; ii < uniquevals.size(); ++ii)
        if (itxyz[2] > uniquevals[ii]) {
          itId[0] = (int)ii;
          break;
        }
      // If the point has not been set, default to the last one
      if (itId[0] < 0) itId[0] = (int)uniquevals.size() - 1;
    }
  }
